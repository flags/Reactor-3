...
def get_mouse_input():
	global MOUSE_POS,MOUSE_1_DOWN

	if mouse_get_status().lbutton:
		if not MOUSE_1_DOWN:
			print 'Click'
			MOUSE_1_DOWN = True
	else:
	...
	MOUSE_POS = mouse_get_status().cx,mouse_get_status().cy
...
while not console_is_window_closed():
	...
	for x in range(MAP_SIZE[0]):
		for y in range(MAP_SIZE[1]):
			...
			if _top_tile_z > CAMERA_Z_LEVEL:
				console_set_char_background(None, x, y, _tile['color'][1]-dark_grey)
				console_set_char_foreground(None, x, y, _tile['color'][0]-dark_grey)
			else:
